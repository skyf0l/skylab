...
- **Everything else is tailnet-only** via self-hosted Headscale. `headscale-proxy`'s `services:` list is the pattern: each entry renders a tailscale node (`<name>.tail.skyf0l.dev`) + an IngressRoute on its zone's in-cluster Traefik entrypoint (`tail-<zone>`). Traefik routes by Host, so a zone (one tag + one entrypoint) is the unit the Headscale ACL can actually isolate; per-service ACLs within a zone are not a boundary. Headscale users are people only; machines are tagged nodes with user-less keys and `tagOwners: []`.
...
- **external-dns Cloudflare**: declare MX priority `0` (anything else churns forever). external-dns only touches records it owns, i.e. ones with a matching `edns-<type>-<name>` heritage TXT — Cloudflare Email Routing's apex MX/SPF are unowned and API-locked, so edits there silently do nothing; delete the record in CF and let external-dns recreate it under its own ownership. Records it created itself, `_dmarc` included, update normally.
- **Headscale Renovate bumps are coupled**: a headscale upgrade runs one-way DB migrations and requires compatible tailscale client versions — bump the tailscale image in `headscale-proxy`/`headscale-exit` in the same change. Forward-fix beats rollback. CLI: machine keys are `preauthkeys create --tags tag:x` with no `--user` (`--user` takes a numeric id, not a name); destroying a user deletes every key it created; stale pre-auth keys crash-loop tailnet pods.
...
- Traefik: a backend restart can leave a stale route serving 502 → `kubectl -n traefik rollout restart daemonset traefik`.
